...
#ifdef GIGAUSBFS
#define FILESYSTEMDRIVER
USBHostMSD* msdp = NULL;
mbed::FATFileSystem* usbp = NULL;
FILE* ifile = NULL;
FILE* ofile = NULL;
DIR* root = NULL;
...
static void gigausbfs_message(const char *s) {
#ifdef GIGAUSBFS_VERBOSE
  consolelog((char*)s);
#endif
}

static void gigausbfs_message_num(int n) {
#ifdef GIGAUSBFS_VERBOSE
  consolelognum(n);
#endif
}
...
/*
 * Arduino GIGA R1 WiFi onboard QSPI LittleFS backend.
 * Uses the official STM32H747 QSPI user-data partition. The current Arduino
 * QSPIFormat layout creates this as MBR partition 4. We deliberately mount
 * partition 4 only so BASIC does not accidentally write to WiFi firmware, OTA,
 * or KVStore partitions.
 */
#ifdef GIGAQSPIFS
#define FILESYSTEMDRIVER
mbed::BlockDevice* qspi_raw_bd = NULL;
mbed::MBRBlockDevice* qspi_user_bd = NULL;
mbed::LittleFileSystem* qspi_fs = NULL;
FILE* ifile = NULL;
FILE* ofile = NULL;
/*
 * Arduino's GIGA Mbed/newlib toolchain supports fopen/stat/remove on mounted
 * LittleFS, but this arm-none-eabi build does NOT support <dirent.h>.
 * So REV11 uses a tiny BASIC file index instead of opendir/readdir.
 * Normal BASIC SAVE adds filenames to _CATALOG.TXT. CATALOG walks that index
 * and verifies every listed file with stat(), so stale entries are ignored.
 */
FILE* qspi_catalog_fp = NULL;
char qspi_catalog_name[FBUFSIZE+1];
uint32_t qspi_catalog_file_size = 0;
const char rootfsprefix[10] = "/fs";
static const char qspi_catalog_index_name[] = "_CATALOG.TXT";
static uint8_t qspi_user_partition = 4;
static int qspi_last_mount_status = 0;

/* Forward declarations needed because the small QSPI catalog helpers live above
 * the generic filesystem section in the original runtime.cpp layout. */
extern uint8_t fsstart;
char* mkfilename(const char* filename);

static void gigaqspifs_message(const char *s) {
#ifdef GIGAQSPIFS_VERBOSE
  consolelog((char*)s);
#endif
}

static void gigaqspifs_message_num(int n) {
#ifdef GIGAQSPIFS_VERBOSE
  consolelognum(n);
#endif
}

static uint8_t gigaqspifs_is_index_name(const char *filename) {
  return (filename && strcmp(filename, qspi_catalog_index_name) == 0);
}

static void gigaqspifs_trim_line(char *s) {
  if (!s) return;
  int n = strlen(s);
  while (n > 0 && (s[n-1] == '\n' || s[n-1] == '\r' || s[n-1] == ' ' || s[n-1] == '\t')) {
    s[--n] = 0;
...
static uint8_t gigaqspifs_file_exists(const char *filename, uint32_t *size_out) {
  struct stat st;
  if (size_out) *size_out = 0;
  if (!filename || !filename[0]) return 0;
  if (stat(mkfilename(filename), &st) == 0 && S_ISREG(st.st_mode)) {
...
static void gigaqspifs_catalog_add(const char *filename) {
  if (!fsstart || !filename || !filename[0]) return;
  if (gigaqspifs_is_index_name(filename)) return;

  char line[FBUFSIZE+1];
  FILE *fp = fopen(mkfilename(qspi_catalog_index_name), "r");
  if (fp) {
    while (fgets(line, sizeof(line), fp)) {
      gigaqspifs_trim_line(line);
  ...
  fp = fopen(mkfilename(qspi_catalog_index_name), "a");
  if (fp) {
    fputs(filename, fp);
    fputc('\n', fp);
    fclose(fp);
...
static uint32_t gigaqspifs_total_kb() {
  if (!qspi_user_bd) return 0;
  uint64_t bytes = (uint64_t)qspi_user_bd->size();
  return (uint32_t)(bytes / 1024ULL);
}

static uint32_t gigaqspifs_used_bytes_indexed() {
  if (!fsstart) return 0;
  uint32_t total = 0;
  char line[FBUFSIZE+1];
  uint32_t sz = 0;
  ...
  while (fgets(line, sizeof(line), fp)) {
    gigaqspifs_trim_line(line);
    if (!line[0]) continue;
    if (gigaqspifs_is_index_name(line)) continue;
    if (gigaqspifs_file_exists(line, &sz)) total += sz;
  }
  fclose(fp);
  if (gigaqspifs_file_exists(qspi_catalog_index_name, &sz)) total += sz;
  return total;
}

static uint32_t gigaqspifs_used_kb() {
  uint32_t b = gigaqspifs_used_bytes_indexed();
  return (b + 1023UL) / 1024UL;
}

static uint32_t gigaqspifs_free_kb_estimate() {
  uint32_t t = gigaqspifs_total_kb();
  uint32_t u = gigaqspifs_used_kb();
  if (u >= t) return 0;
  return t - u;
}

extern "C" uint32_t gigaqspi_total_kb_public() { return gigaqspifs_total_kb(); }
extern "C" uint32_t gigaqspi_used_kb_public() { return gigaqspifs_used_kb(); }
extern "C" uint32_t gigaqspi_free_kb_public() { return gigaqspifs_free_kb_estimate(); }
extern "C" uint32_t gigaqspi_used_bytes_public() { return gigaqspifs_used_bytes_indexed(); }
#endif
...
#if defined(RP2040LITTLEFS) || defined(ESPSPIFFS) || defined(ESP32FAT) || defined(ARDUINOSD) || defined(GIGAUSBFS) || defined(GIGAQSPIFS) || defined(ESPSDMMC)
...
char* mkfilename(const char* filename) {
	...
	if (i > 0 && tmpfilename[i-1] != '/') tmpfilename[i++]='/';
...
#define SDPIN
  ...
  fsstart = 0;
  gigausbfs_message("USB drive: searching USB-A host port...\n");

/* close any previously open files/directories before restarting USB storage */
  if (ifile) { fclose(ifile); ifile = NULL; }
  if (ofile) { fclose(ofile); ofile = NULL; }
  if (root)  { (void) closedir(root); root = NULL; }

/* unmount/delete old objects so CALL 1 can rescan after a stick is inserted */
  if (usbp) { (void) usbp->unmount(); delete usbp; usbp = NULL; }
  if (msdp) { delete msdp; msdp = NULL; }

/* power-cycle the GIGA USB-A port */
...
/* create the USB mass-storage object and FAT filesystem object */ 
  msdp = new USBHostMSD();
  usbp = new mbed::FATFileSystem("usb"); 

/* connect and mount: wait up to about 6 seconds for a FAT32 USB stick */
  while (!msdp->connect() && attempts < 24) {
    bdelay(250);
    attempts++;
  }
  ...
  if (msdp->connected()) {
    ...
    fsstart = !usbfsstat;
    if (fsstart) {
      gigausbfs_message("USB drive: mounted OK at /usb. SAVE/LOAD/CATALOG enabled.\n");
    } else {
      gigausbfs_message("USB drive: found, but FAT mount failed. Mount status ");
      gigausbfs_message_num(usbfsstat);
      gigausbfs_message(". Use FAT32 and try CALL 1.\n");
    }
  } else {
    gigausbfs_message("USB drive: not found. Insert FAT32 stick and type CALL 1 to rescan.\n");
  }
#endif
#ifdef GIGAQSPIFS
  fsstart = 0;
  qspi_last_mount_status = -9999;
  gigaqspifs_message("GIGA onboard QSPI: mounting LittleFS user partition /fs...\n");

  /* Close open POSIX handles before remounting. CALL 1 can use this safely. */
  if (ifile) { fclose(ifile); ifile = NULL; }
  if (ofile) { fclose(ofile); ofile = NULL; }
  if (qspi_catalog_fp) { fclose(qspi_catalog_fp); qspi_catalog_fp = NULL; }

  if (qspi_fs) { (void) qspi_fs->unmount(); delete qspi_fs; qspi_fs = NULL; }
  if (qspi_user_bd) { delete qspi_user_bd; qspi_user_bd = NULL; }

  qspi_raw_bd = mbed::BlockDevice::get_default_instance();
  if (!qspi_raw_bd) {
    qspi_last_mount_status = -1;
    gigaqspifs_message("GIGA onboard QSPI: no default BlockDevice found.\n");
  } else {
    qspi_user_bd = new mbed::MBRBlockDevice(qspi_raw_bd, qspi_user_partition);
    qspi_fs = new mbed::LittleFileSystem("fs");
    qspi_last_mount_status = qspi_fs->mount(qspi_user_bd);
    fsstart = (qspi_last_mount_status == 0);
    if (fsstart) {
      gigaqspifs_message("GIGA onboard QSPI: mounted OK at /fs. SAVE/LOAD/CATALOG enabled.\n");
    } else {
      gigaqspifs_message("GIGA onboard QSPI: mount failed on partition ");
      gigaqspifs_message_num(qspi_user_partition);
      gigaqspifs_message(" with status ");
      gigaqspifs_message_num(qspi_last_mount_status);
      gigaqspifs_message(". Run Arduino QSPIFormat with LittleFS user partition, or use FDISK 0 if partition 4 already exists.\n");
    }
  }
#endif
#ifdef ARDUINOEFS
...
#ifdef GIGAQSPIFS
  if (c == 2) return qspi_user_partition;
  if (c == 3) return (uint8_t)(qspi_last_mount_status & 0xff);
#endif
#endif
  return 0;
}
...
#endif
#if defined(RP2040LITTLEFS) || defined(GIGAUSBFS) || defined(GIGAQSPIFS)
...
#ifdef GIGAQSPIFS
	ofile=fopen(mkfilename(filename), m);
	if (ofile && *m == 'w') gigaqspifs_catalog_add(filename);
	return ofile != 0; 
#endif
#ifdef ARDUINOEFS
...
#endif
#ifdef GIGAQSPIFS
	qspi_catalog_name[0] = 0;
	qspi_catalog_file_size = 0;
	if (qspi_catalog_fp) { fclose(qspi_catalog_fp); qspi_catalog_fp = NULL; }
	qspi_catalog_fp = fopen(mkfilename(qspi_catalog_index_name), "r");
#endif
#ifdef ARDUINOEFS
	EFS.dirp=0;
#endif
...
#endif
#ifdef GIGAQSPIFS
	if (!qspi_catalog_fp) return 0;
	char line[FBUFSIZE+1];
	uint32_t sz = 0;
	while (fgets(line, sizeof(line), qspi_catalog_fp)) {
		gigaqspifs_trim_line(line);
		if (!line[0]) continue;
		if (gigaqspifs_is_index_name(line)) continue;
		if (gigaqspifs_file_exists(line, &sz)) {
			strncpy(qspi_catalog_name, line, FBUFSIZE);
			qspi_catalog_name[FBUFSIZE] = 0;
			qspi_catalog_file_size = sz;
			return 1;
		}
	}
	return 0;
...
#ifdef GIGAUSBFS
  /*
   * Mbed FAT may report DT_UNKNOWN in dirent.d_type. Use stat() instead and
   * fall back to true for normal names so small FAT32 USB sticks list correctly.
   */
  if (!file) return 0;
  if (file->d_name[0] == '.') return 0;
  struct stat st;
  if (stat(mkfilename(file->d_name), &st) == 0) {
    return S_ISREG(st.st_mode);
  }
  return 1;
#endif
#ifdef GIGAQSPIFS
  return (qspi_catalog_name[0] != 0);
#endif
#ifdef ARDUINOEFS
	return 1;
#endif	
...
/* to be done */
  ...
  if (file) {
    struct stat st;
    if (stat(mkfilename(file->d_name), &st) == 0) return (uint32_t)st.st_size;
  }
#endif
#ifdef GIGAQSPIFS
  return qspi_catalog_file_size;
#endif
#ifdef ARDUINOEFS
...
#endif
#ifdef GIGAQSPIFS
  if (!qspi_raw_bd) qspi_raw_bd = mbed::BlockDevice::get_default_instance();
  if (!qspi_user_bd && qspi_raw_bd) qspi_user_bd = new mbed::MBRBlockDevice(qspi_raw_bd, qspi_user_partition);
  if (!qspi_fs) qspi_fs = new mbed::LittleFileSystem("fs");
  if (qspi_fs && qspi_user_bd) {
    (void) qspi_fs->unmount();
    if (qspi_fs->reformat(qspi_user_bd) == 0) fsstart=1; else fsstart=0;
  } else {
    fsstart=0;
  }
  return;
