...
/* GIGA BASIC Mean Machine public QSPI disk helpers. */
#ifdef GIGAQSPIFS
extern "C" uint32_t gigaqspi_total_kb_public();
extern "C" uint32_t gigaqspi_used_kb_public();
extern "C" uint32_t gigaqspi_free_kb_public();
extern "C" uint32_t gigaqspi_used_bytes_public();
#endif

/* USR with arguments > 31 calls this */
number_t usrfunction(address_t i, number_t v) {
#ifdef GIGAQSPIFS
  /* GIGA QSPI diagnostics:
     USR(40,0) = filesystem driver compiled in
     USR(40,1) = QSPI LittleFS mounted
     USR(40,2) = user partition number, normally 4
     USR(40,3) = last mount status low byte
  */
  if (i == 40) {
    if (v == 0) return fsstat(0);
    if (v == 1) return fsstat(1);
    if (v == 2) return fsstat(2);
    if (v == 3) return fsstat(3);
    if (v == 4) return (number_t)gigaqspi_used_kb_public();
    if (v == 5) return (number_t)gigaqspi_free_kb_public();
    if (v == 6) return (number_t)gigaqspi_total_kb_public();
  }
#endif
  return 0;
}
...
#ifdef GIGAQSPIFS
static void giga_qspi_write_cstr(const char *s) {
  while (*s) filewrite(*s++);
}

static void giga_qspi_diag_status() {
  outcr();
  outsc("GIGA QSPI LITTLEFS STATUS"); outcr();
  outsc("Driver compiled:       "); outnumber(fsstat(0)); outcr();
  outsc("Mounted:               "); outnumber(fsstat(1)); outcr();
  outsc("User partition number: "); outnumber(fsstat(2)); outcr();
  outsc("Last mount status low byte: "); outnumber(fsstat(3)); outcr();
  outsc("Mount path: /fs"); outcr();
}

static void giga_qspi_diag_write_read() {
  char ch;
  int count;

  outcr();
  outsc("GIGA QSPI RAW WRITE/READ DIAGNOSTIC"); outcr();
  outsc("Driver compiled: "); outnumber(fsstat(0)); outcr();
  outsc("QSPI mounted:    "); outnumber(fsstat(1)); outcr();

  if (!fsstat(1)) {
    outsc("FAIL: QSPI LittleFS filesystem is not mounted."); outcr();
    outsc("Try CALL 1. If still failed, run Arduino QSPIFormat with LittleFS user data."); outcr();
    outsc("If partition 4 exists but is blank, FDISK 0 can format only the BASIC filesystem."); outcr();
    return;
  }

  removefile("QSPITST.TXT");

  outsc("Opening QSPITST.TXT for write..."); outcr();
  if (!ofileopen("QSPITST.TXT", "w")) {
    outsc("FAIL: write open failed."); outcr();
    return;
  }

  giga_qspi_write_cstr("GIGA QSPI WRITE TEST\r\n");
  giga_qspi_write_cstr("IF YOU CAN READ THIS, ONBOARD FLASH WRITE WORKS.\r\n");
  ofileclose();
  outsc("Write close OK."); outcr();

  outsc("Opening QSPITST.TXT for read..."); outcr();
  if (!ifileopen("QSPITST.TXT")) {
    outsc("FAIL: read open failed after write."); outcr();
    return;
  }

  outsc("READBACK BEGIN"); outcr();
  count = 0;
  while (fileavailable() && count < 200) {
    ch = fileread();
    if ((unsigned char)ch == 255) break;
    if (ch == '\r') {
      /* skip CR */
    } else if (ch == '\n') {
      outcr();
  ...
  ifileclose();
  outcr();
  outsc("READBACK END"); outcr();
  outsc("PASS: raw onboard QSPI write/read path works."); outcr();
}

static void giga_qspi_diag_catalog() {
  int seen;

  outcr();
  outsc("GIGA QSPI RAW CATALOG DIAGNOSTIC"); outcr();
  outsc("QSPI mounted: "); outnumber(fsstat(1)); outcr();
  ...
  if (!seen) {
    outsc("No directory entries found. That is OK if the filesystem is empty."); outcr();
  }
}

static void giga_qspi_make_sample_basic() {
  outcr();
  outsc("Writing SAMPLE.BAS directly to onboard QSPI flash..."); outcr();
  ...
  removefile("SAMPLE.BAS");
  if (!ofileopen("SAMPLE.BAS", "w")) {
    outsc("FAIL: could not open SAMPLE.BAS for write."); outcr();
    return;
  }

  giga_qspi_write_cstr("10 PRINT \"SAMPLE.BAS LOADED FROM ONBOARD QSPI\"\r\n");
  giga_qspi_write_cstr("20 FOR I=1 TO 10\r\n");
  giga_qspi_write_cstr("30 PRINT I,I*I\r\n");
  giga_qspi_write_cstr("40 NEXT I\r\n");
  ofileclose();

  outsc("PASS: SAMPLE.BAS written."); outcr();
  outsc("Now try: NEW"); outcr();
  outsc("          LOAD \"SAMPLE.BAS\""); outcr();
  outsc("          LIST"); outcr();
  outsc("          RUN"); outcr();
}


static void giga_basic_disk_space() {
  uint32_t total = gigaqspi_total_kb_public();
  uint32_t used = gigaqspi_used_kb_public();
  uint32_t free_est = gigaqspi_free_kb_public();
  uint32_t used_bytes = gigaqspi_used_bytes_public();

  outcr();
  outsc("GIGA BASIC MEAN MACHINE DISK SPACE"); outcr();
  outsc("Driver compiled:     "); outnumber(fsstat(0)); outcr();
  outsc("Mounted:             "); outnumber(fsstat(1)); outcr();
  outsc("Disk:                onboard QSPI LittleFS /fs"); outcr();
  outsc("Total KB:            "); outnumber(total); outcr();
  outsc("Cataloged used KB:   "); outnumber(used); outcr();
  outsc("Cataloged used bytes:"); outnumber(used_bytes); outcr();
  outsc("Estimated free KB:   "); outnumber(free_est); outcr();
  outsc("Note: free space is estimated from the BASIC catalog index."); outcr();
}

static void giga_basic_disk_commands() {
  outcr();
  outsc("GIGA BASIC MEAN MACHINE DISK COMMANDS"); outcr();
  outsc("SAVE \"NAME.BAS\"    save current BASIC program"); outcr();
  outsc("LOAD \"NAME.BAS\"    load a BASIC program"); outcr();
  outsc("CATALOG            list saved files and sizes"); outcr();
  outsc("DELETE \"NAME.BAS\"  delete a saved file"); outcr();
  outsc("NEW                clear program memory before load"); outcr();
  outsc("FDISK 0            format BASIC QSPI filesystem only"); outcr();
  outsc("CALL 40            QSPI mount/status"); outcr();
  outsc("CALL 44            disk space report"); outcr();
  outsc("CALL 45            this command sheet"); outcr();
  outsc("CALL 46            write AUTOEXEC.BAS menu"); outcr();
  outsc("CALL 47            write example program library"); outcr();
  outsc("CALL 48            revision/about information"); outcr();
  outsc("USR(40,4)          cataloged used KB"); outcr();
  outsc("USR(40,5)          estimated free KB"); outcr();
  outsc("USR(40,6)          total QSPI partition KB"); outcr();
}

static void giga_basic_make_autoexec() {
  outcr();
  outsc("Writing AUTOEXEC.BAS menu to onboard QSPI..."); outcr();
  if (!fsstat(1)) { outsc("FAIL: QSPI is not mounted."); outcr(); return; }
  removefile("AUTOEXEC.BAS");
  if (!ofileopen("AUTOEXEC.BAS", "w")) { outsc("FAIL: could not open AUTOEXEC.BAS."); outcr(); return; }
  giga_qspi_write_cstr("10 CLS\r\n");
  giga_qspi_write_cstr("20 PRINT \"GIGA BASIC MEAN MACHINE\"\r\n");
  giga_qspi_write_cstr("30 PRINT \"1 MANDELBROT\"\r\n");
  giga_qspi_write_cstr("40 PRINT \"2 PRIME TEST\"\r\n");
  giga_qspi_write_cstr("50 PRINT \"3 DISK TEST\"\r\n");
  giga_qspi_write_cstr("60 PRINT \"4 CATALOG\"\r\n");
  giga_qspi_write_cstr("70 PRINT \"5 BASIC PROMPT\"\r\n");
  giga_qspi_write_cstr("80 INPUT \"SELECT\";A\r\n");
  giga_qspi_write_cstr("90 IF A=1 THEN LOAD \"MANDEL.BAS\"\r\n");
  giga_qspi_write_cstr("100 IF A=1 THEN RUN\r\n");
  giga_qspi_write_cstr("110 IF A=2 THEN LOAD \"PRIME.BAS\"\r\n");
  giga_qspi_write_cstr("120 IF A=2 THEN RUN\r\n");
  giga_qspi_write_cstr("130 IF A=3 THEN LOAD \"DISKTEST.BAS\"\r\n");
  giga_qspi_write_cstr("140 IF A=3 THEN RUN\r\n");
  giga_qspi_write_cstr("150 IF A=4 THEN CATALOG\r\n");
  giga_qspi_write_cstr("160 IF A=4 THEN GOTO 20\r\n");
  giga_qspi_write_cstr("170 PRINT \"READY. TYPE HELP OR CALL 45.\"\r\n");
  ofileclose();
  outsc("PASS: AUTOEXEC.BAS written. Try LOAD \"AUTOEXEC.BAS\" then RUN."); outcr();
}

static void giga_basic_write_library() {
  outcr();
  outsc("Writing starter BASIC library to onboard QSPI..."); outcr();
  if (!fsstat(1)) { outsc("FAIL: QSPI is not mounted."); outcr(); return; }

  removefile("HELLO.BAS");
  if (ofileopen("HELLO.BAS", "w")) {
    giga_qspi_write_cstr("10 PRINT \"HELLO FROM GIGA BASIC MEAN MACHINE\"\r\n20 PRINT \"QSPI SAVE/LOAD WORKS\"\r\n");
    ofileclose();
  }
  removefile("DISKTEST.BAS");
  if (ofileopen("DISKTEST.BAS", "w")) {
    giga_qspi_write_cstr("10 PRINT \"DISK TEST FROM QSPI\"\r\n20 FOR I=1 TO 10\r\n30 PRINT I,I*I,I*I*I\r\n40 NEXT I\r\n50 CATALOG\r\n");
    ofileclose();
  }
  removefile("PRIME.BAS");
  if (ofileopen("PRIME.BAS", "w")) {
    giga_qspi_write_cstr("10 PRINT \"PRIME TESTER\"\r\n20 INPUT \"MAX\";N\r\n30 FOR X=2 TO N\r\n40 P=1\r\n50 FOR D=2 TO INT(SQR(X))\r\n60 IF X/D=INT(X/D) THEN P=0\r\n70 NEXT D\r\n80 IF P=1 THEN PRINT X;\" \";\r\n90 NEXT X\r\n100 PRINT\r\n");
    ofileclose();
  }
  outsc("PASS: HELLO.BAS, DISKTEST.BAS, and PRIME.BAS written."); outcr();
  outsc("Use CATALOG, LOAD \"HELLO.BAS\", RUN."); outcr();
}

static void giga_basic_about() {
  outcr();
  outsc("GIGA BASIC MEAN MACHINE REV11"); outcr();
  outsc("Based on Stefan's Basic / slviajero tinybasic."); outcr();
  outsc("This fork/package was customized for Arduino GIGA R1 WiFi."); outcr();
  outsc("Major additions: onboard QSPI LittleFS disk, BASIC catalog index,"); outcr();
  outsc("SAVE/LOAD/CATALOG/DELETE workflow, diagnostics, disk-space report,"); outcr();
  outsc("starter program library, and GIGA-specific setup documentation."); outcr();
  outsc("Storage target: official QSPI user-data partition mounted at /fs."); outcr();
}
#endif
...
#ifdef GIGAQSPIFS
  if (i == 40) { giga_qspi_diag_status(); return; }
  if (i == 41) { giga_qspi_diag_write_read(); return; }
  if (i == 42) { giga_qspi_diag_catalog(); return; }
  if (i == 43) { giga_qspi_make_sample_basic(); return; }
  if (i == 44) { giga_basic_disk_space(); return; }
  if (i == 45) { giga_basic_disk_commands(); return; }
  if (i == 46) { giga_basic_make_autoexec(); return; }
  if (i == 47) { giga_basic_write_library(); return; }
  if (i == 48) { giga_basic_about(); return; }
#endif
  return;
}
...
void displaybanner() {
  ...
  outcr();
#ifdef GIGA_BASIC_MEANMACHINE_REV11
  outsc("GIGA BASIC MEAN MACHINE REV11 - onboard flash build"); outcr();
#endif
#endif
}
